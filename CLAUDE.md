...
```
Source/
├── RimMindStorytellerMod.cs                     Mod 入口，注册 Harmony/SettingsTab/Cooldown，初始化设置
├── Storyteller/
...
// 3. 若 RimMindAPI.CanTriggerDialogue → 随机选殖民者 → RimMindAPI.TriggerDialogue(pawn, incidentContext)
```
...
[输出] 只返回 JSON：{"defName":"...","reason":"...","params":{...},"chain":{...}}
[兜底] 无法选择时返回第一个候选
...
| 设置 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| enableIntervalTrigger | bool | true | 启用 MTB 周期触发 |
